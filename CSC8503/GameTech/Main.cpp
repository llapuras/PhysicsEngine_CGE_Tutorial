...
int main() {
	...
	float lastUpdateTime = w->GetTimer()->GetTotalTimeSeconds();
	float frameCount = 0;
	float fps = 0;

	while (w->UpdateWindow() && !Window::GetKeyboard()->KeyDown(KeyboardKeys::ESCAPE)) {
		float dt = w->GetTimer()->GetTimeDeltaSeconds();
		float dtSecond = w->GetTimer()->GetTotalTimeSeconds() - lastUpdateTime;
		...
		{
			fps = frameCount;
			frameCount = 0;//frame¹éÁã
			lastUpdateTime = w->GetTimer()->GetTotalTimeSeconds();
		}
		...
		w->SetTitle("Gametech frame time:" + std::to_string(1000.0f * dt) + " FPS:" + std::to_string(fps));
